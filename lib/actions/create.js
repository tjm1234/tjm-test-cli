...
async function saveTpl() {
  const tplDir = path.join(os.homedir(), '.tjm-templates')
  await fs.ensureDir(tplDir)
  const defaultTplDir = path.join(tplDir, 'template-default')
  console.log(defaultTplDir,'defaultTplDirdefaultTplDir')
  if (fs.existsSync(path.join(defaultTplDir, '.git'))) {
    // 模板已存在则 pull 最新代码
    await git.pull(defaultTplDir)
  } else {
    // 模板不存在则 clone 到本地
    await git.clone('https://github.com/tjm1234/template-default.git', {
      workdir: tplDir,
    })
  }
...
async function renderTpl(appName, tplDir) {
  ...
  const appTplDir = path.join(tplDir, 'templates/skeleton')
